  ...
  return (
    <>
      <div className="relative h-[190px] overflow-hidden bg-zinc-100">
        ...
        />
        <div className="absolute top-3 left-3 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-semibold text-emerald-700 backdrop-blur-sm">
      ...
      <div className="space-y-3.5 p-4">
        ...
        <div className="grid grid-cols-3 divide-x divide-zinc-200 border-t border-zinc-200 pt-3">
          <div className="pr-2">
            <p className="text-[9px] font-semibold tracking-wide text-zinc-400 uppercase">
              Estimado
            </p>
          ...
          <div className="px-3">
            ...
            <p className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-zinc-900">
...
function SwipeableCard({ provider, onAccept, onReject }: SwipeableCardProps) {
  ...
  return (
    <motion.article
      ...
      className="absolute inset-x-0 top-0 z-30 cursor-grab touch-pan-y overflow-hidden rounded-[24px] border border-zinc-200 bg-white active:cursor-grabbing"
    >
      <motion.div
...
export function CardStack({
  ...
  if (cards.length === 0) {
    return (
      <div
        className={cn(
          "flex h-[480px] flex-col items-center justify-center rounded-[24px] border border-dashed border-zinc-300 bg-white px-8 text-center",
          className
        )}
      >
  ...
  return (
    <div className={cn("relative h-[480px]", className)}>
      {cards.slice(1, 3).map((provider, index) => (
        <div
          key={provider.id}
          className="absolute inset-x-3 top-0 h-[470px] rounded-[24px] border border-zinc-200 bg-white"
          style={{
